...
foreach ($media_types as $type) {
    ...
    // Prepare SQL query to fetch watched anime for the logged-in user
    $sql = "SELECT a.aid, a.anime_name, a.anime_poster_link 
            FROM anime a 
            JOIN review r ON a.aid = r.media_id 
            WHERE r.uid = ? AND r.media_id >= 30001 AND r.media_id < 40000";
                ...
                foreach ($media as $type => $content) {
                    ...
                    <h2>Watched {$media_type}s</h2>
                        ...
                        foreach ($content as $item) {
                            $id = $item['aid'];
                            $poster_link = $item['anime_poster_link'];
                            $name = ucwords($item['anime_name']);
