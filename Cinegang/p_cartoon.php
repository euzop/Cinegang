...
foreach ($media_types as $type) {
    ...
    // Prepare SQL query to fetch watched cartoon for the logged-in user
    $sql = "SELECT a.cid, a.cartoon_name, a.cartoon_poster_link 
            FROM cartoon a 
            JOIN review r ON a.cid = r.media_id 
            WHERE r.uid = ? AND r.media_id >= 40001 AND r.media_id < 50000";
        ...
        <div class="container py-5">
            <div class="media-wrapper">
                ...
                foreach ($media as $type => $content) {
                    if ($type === 'cartoon' && !empty($content)) {
                        $media_type = ucwords($type);
                        echo "<div class=\"media-heading\">
                    <h2>Watched {$media_type}s</h2>
                  </div>";

                        echo "<div class=\"media-items\">";
                        foreach ($content as $item) {
                            $id = $item['cid'];
                            $poster_link = $item['cartoon_poster_link'];
                            $name = ucwords($item['cartoon_name']);
