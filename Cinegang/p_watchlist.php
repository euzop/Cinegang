...
// Prepare SQL queries to fetch watchlist items for the logged-in user
$queries = [
    'movie' => "SELECT m.mid as id, m.movie_name as name, m.movie_poster_link as poster_link 
                FROM movie m 
                JOIN watchlist w ON m.mid = w.media_id 
                WHERE w.uid = ? AND w.media_id >= 10001 AND w.media_id < 20000",
    'series' => "SELECT s.sid as id, s.series_name as name, s.series_poster_link as poster_link 
                 FROM series s 
                 JOIN watchlist w ON s.sid = w.media_id 
                 WHERE w.uid = ? AND w.media_id >= 20001 AND w.media_id < 30000",
    'anime' => "SELECT a.aid as id, a.anime_name as name, a.anime_poster_link as poster_link 
                FROM anime a 
                JOIN watchlist w ON a.aid = w.media_id 
                WHERE w.uid = ? AND w.media_id >= 30001 AND w.media_id < 40000",
    'cartoon' => "SELECT c.cid as id, c.cartoon_name as name, c.cartoon_poster_link as poster_link 
                  FROM cartoon c 
                  JOIN watchlist w ON c.cid = w.media_id 
                  WHERE w.uid = ? AND w.media_id >= 40001 AND w.media_id < 50000"
];
