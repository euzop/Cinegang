...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['account_form'])) {
    ...
    if (!empty($new_password)) {
        if (empty($current_password) || !password_verify($current_password, $user['password'])) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssssi", $name, $nickname, $profile_pic, $bio, $mobile, $email, $hashed_new_password, $uid);
            if (!$stmt->execute()) {
...
<body>
<!-- Debugging Outputs -->
<div style="background-color: #f8d7da; color: #721c24; padding: 10px; margin: 20px 0;">
    <strong>Debugging Outputs:</strong><br>
    <?php if (!empty($errors['password'])): ?>
        Current Password Error: <?php echo $errors['password']; ?><br>
    <?php endif; ?>
    <?php if (!empty($errors['new_password'])): ?>
        New Password Error: <?php echo $errors['new_password']; ?><br>
    <?php endif; ?>
    Entered Current Password: <?php echo isset($current_password) ? htmlspecialchars($current_password) : 'Not set'; ?><br>
    Entered New Password: <?php echo isset($new_password) ? htmlspecialchars($new_password) : 'Not set'; ?><br>
    Entered Confirm Password: <?php echo isset($confirm_password) ? htmlspecialchars($confirm_password) : 'Not set'; ?><br>
    Stored Password Hash: <?php echo htmlspecialchars($user['password']); ?><br>
</div>
...
<div class="account-box">
    <form action="account.php" method="POST" class="account-box-form">
        <h3 class="account-box-title">Account Settings</h3>
