...
foreach ($media_types as $type) {
    ...
    // Prepare SQL query to fetch watched series for the logged-in user
    $sql = "SELECT a.sid, a.series_name, a.series_poster_link 
            FROM series a 
            JOIN review r ON a.sid = r.media_id 
            WHERE r.uid = ? AND r.media_id >= 20001 AND r.media_id < 30000";
                ...
                foreach ($media as $type => $content) {
                    ...
                    <h2>Watched {$media_type}</h2>
                        ...
                        foreach ($content as $item) {
                            $id = $item['sid'];
                            $poster_link = $item['series_poster_link'];
                            $name = ucwords($item['series_name']);
